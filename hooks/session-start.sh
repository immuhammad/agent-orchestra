...
echo '{"additionalContext": "HARNESS ACTIVE. Read .harness/handoff.md before any work. Quota rule: at 80% of any window, stop, update handoff, ask Ahmad about fallback switch. Ahmad: run /usage to check Claude quota, agy quota screen for Google pool. Skills auto-use by role (see AGENTS.md Skills table): Builder -- test-driven-development, systematic-debugging, verification-before-completion, using-git-worktrees; Orchestra -- brainstorming, writing-plans (+ fan-out skills, Orchestra-only). Inbox .msg bodies: caveman mode; handoff.md/decisions.log/PR descriptions/issue comments stay full English."}'
