...
# the hook now gates its nag on ORC_ROLE=orchestra -- every
...
run_hook() {
  ...
  if [ -z "$role" ]; then
    ...
    # and turn this into whatever role the pane happens to be (found
    # running the suite from the orchestra pane).
...
echo "== builder-role stop with the SAME stale handoff.md does NOT nag -- handoff.md is Orchestra's file, Builder clobbered it 3x =="
...
STATUS=$?
if [ "$STATUS" -eq 0 ] && [ -z "$OUT" ]; then
  pass "builder-role stop with a stale handoff.md is silent -- no nag, no block"
else
  fail "CORRECTNESS REGRESSION: builder-role stop should never nag about handoff.md (got status=$STATUS): $OUT"
fi

echo "== unknown role (no ORC_ROLE at all -- a session started outside the room) does NOT nag =="
...
STATUS=$?
if [ "$STATUS" -eq 0 ] && [ -z "$OUT" ]; then
  pass "no ORC_ROLE at all (session started outside the room) is silent -- treated as unknown, never nagged"
else
  fail "CORRECTNESS REGRESSION: a session with no ORC_ROLE should never nag about handoff.md (got status=$STATUS): $OUT"
fi

echo "== an unrecognized role string (neither orchestra nor builder) does NOT nag -- unknown, not orchestra =="
OUT="$(run_hook scribe)"
STATUS=$?
if [ "$STATUS" -eq 0 ] && [ -z "$OUT" ]; then
  pass "an unrecognized ORC_ROLE value is silent -- only an EXACT 'orchestra' match nags"
else
  fail "CORRECTNESS REGRESSION: an unrecognized role should never nag about handoff.md (got status=$STATUS): $OUT"
fi

echo "== orchestra WITH handoff.md updated this session: still no nag (role-gating doesn't override the real up-to-date check) =="
touch "$SANDBOX/.harness/.session-start"
sleep 1
printf '# HANDOFF\n\n## Current state\nfresh again\n' > "$SANDBOX/.harness/handoff.md"
OUT="$(run_hook orchestra)"
STATUS=$?
if [ "$STATUS" -eq 0 ] && [ -z "$OUT" ]; then
  pass "orchestra with a freshly-updated handoff.md still passes silently"
else
  fail "expected exit 0 and no output for orchestra with a fresh handoff.md (got status=$STATUS): $OUT"
fi

echo "== T31: handoff.md over 100 lines WARNS but does not block =="
touch "$SANDBOX/.harness/.session-start"
sleep 1
...
echo "== ORC_ONESHOT=1 exempts a one-shot/headless session from the handoff Stop hook =="
...
echo "== without ORC_ONESHOT, the same stale state still blocks (no accidental widening) =="
OUT="$(run_hook)"
STATUS=$?
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -q "not updated this session"; then
...
echo "== SECURITY (agy's probe-3): the ORC_ONESHOT exemption is safe only because guard-write.sh blocks the spoof it depends on =="
...
# guard-write.sh, not in this hook -- verify it end-to-end here
