#!/bin/bash
# tests/check-hook-wiring.test.sh — lib/check-hook-wiring.sh
...
# This is the only thing that would have caught the live incident: the
# gate shipped but was never wired into this room's config, and
...
echo "== missing settings.json fails LOUDLY (exit 2 + explicit message), not silently =="
...
echo "== this is EXACTLY the incident -- a settings.json that wires guard.sh/guard-write.sh but NOT quota-stop-gate.sh fails LOUDLY =="
...
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -qi "quota-stop-gate"; then
  pass "reproduces the incident: a settings.json missing the gate hook fails loudly, naming quota-stop-gate.sh"
else
  fail "expected exit 2 naming quota-stop-gate.sh as missing (got status=$STATUS): $OUT"
fi

echo "== a settings.json wiring quota-stop-gate.sh but NOT room-branch-gate.sh fails LOUDLY, naming it =="
...
echo "== the ACTUAL incident shape -- a settings.json passing every hardcoded check (quota-stop-gate + room-branch-gate + guard/guard-write) but missing ALL THREE pane-state.sh hooks (never in the old hardcoded list) is caught, naming pane-state.sh =="
...
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -qi "pane-state.sh"; then
  pass "reproduces the historical incident -- a config missing pane-state.sh (never in the old hardcoded list) is now caught, naming it"
else
  fail "CORRECTNESS REGRESSION: expected exit 2 naming pane-state.sh as missing (got status=$STATUS): $OUT"
fi
...
if [ "$MISSING_COUNT" -ge 3 ]; then
  pass "all three missing pane-state.sh hooks (UserPromptSubmit/PreToolUse/Stop) are named individually, not just one"
else
  fail "expected at least 3 pane-state.sh mentions (one per missing event), got $MISSING_COUNT: $OUT"
fi

echo "== an ALTERED hook command (same script, different invocation) fails LOUDLY, distinguished from a MISSING one =="
ALTERED_TMP="$(mktemp)"
...
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -qi "ALTERED" && echo "$OUT" | grep -q "guard.sh"; then
  pass "an altered guard.sh invocation is caught and reported as ALTERED, naming guard.sh"
else
  fail "expected exit 2 reporting an ALTERED guard.sh hook (got status=$STATUS): $OUT"
fi
rm -f "$ALTERED_TMP"

echo "== an EXTRA live-only hook (not in the template) passes with a NOTE, does not fail =="
EXTRA_TMP="$(mktemp)"
...
STATUS=$?
if [ "$STATUS" -eq 0 ]; then
  pass "an extra live-only hook does not fail the check (a room may legitimately layer extras)"
else
...
if echo "$OUT" | grep -qi "NOTE.*room-specific-extra.sh\|room-specific-extra.sh.*NOTE\|extra hook"; then
  pass "the extra hook is still surfaced as a NOTE, not silently ignored"
else
  fail "expected a NOTE about the extra hook even though it passes: $OUT"
fi
rm -f "$EXTRA_TMP"

echo "== kills the hardcoded list -- adding a brand-new hook to the template is caught automatically, no doctor code change needed =="
...
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -qi "totally-hypothetical-future-hook.sh"; then
  pass "a hook this script has never heard of by name is still caught, purely from diffing against the template"
else
...
echo "== missing/unreadable TEMPLATE file fails loudly too (not just a missing live file) =="
...
if [ "$STATUS" -eq 2 ] && echo "$OUT" | grep -qi "template.*does not exist"; then
  pass "a missing template file fails loudly with an explicit message"
else
